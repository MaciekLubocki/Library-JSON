...
def not_found(error):
    return make_response(jsonify({'error': 'Not found', 'status_code': 404}), 404)  # noqa


@app.route("/api/v1/items/", methods=["POST"])
def create_item():
    if not request.json or not 'title' in request.json:  # noqa
        abort(400)
    item = {
...
def bad_request(error):
    return make_response(jsonify({'error': 'Bad request', 'status_code': 400}), 400)  # noqa
